...
public class ProveedorPotencialAprobadoNotificacion extends NotificarMail {
    ...
    private final String TEMPLATE_MIGRADO = "com/incloud/hcp/templates/portal/TmpProveedorMigrado.html";
//    private final String TEMPLATERECHAZO = "com/incloud/hcp/templates/portal/TmpProveedorPotencialRechazado.html";
    private final String ASUNTO = "Registro de solicitud aprobado";
    private final String ASUNTO_MIGRADO = "Bienvenido al Portal de Proveedores JRC";
...
//    }
    ...
    public String enviar(MailSetting mailSetting, PreRegistroProveedor preRegistro){
...
//        body = body + "Gracias por registrarse en el IProvider de Topitop&#44;<br/>";
//        body = body + "por favor ingrese al siguiente enlace para completar su información:<br/>";
...
//        body = body + "</html>";
        ...
        try {
            ...
            //this.enviarCorreoSMTP(preRegistro.getEmail(), ASUNTO, content, mailSetting);
        } catch (Exception ex) {
    ...
    public String enviarMigrado(MailSetting mailSetting, Proveedor proveedor, String password){
        String contactoProveedor = null;
        if (proveedor.getRazonSocial() != null) {
            contactoProveedor = proveedor.getRazonSocial();
        } else {
            contactoProveedor = "Usuario";
        }
        ...
        try {
            ...
            //context.put("vendor_call_action",LOGO_CAL );
            context.put("vendor_footer", LOGO_FOOT);
            context.put("urlportalSap", urlPortalSap);
            context.put("urlportal", urlPortal);

            String content = Optional.ofNullable(TEMPLATE_MIGRADO)
                    .map(url -> url + "")
                    .map(template -> {
                        int i = 0;
                        return getContentMail(context, template);
            ...
            htmlMail.setHostName("smtp-mail.outlook.com");
            htmlMail.setSmtpPort(587);
            htmlMail.setAuthenticator(new DefaultAuthenticator(mailSetting.getUser(), mailSetting.getPassword()));
            htmlMail.addTo(proveedor.getEmail());
            htmlMail.setFrom(mailSetting.getEmailFrom(), mailSetting.getNameFrom());
            htmlMail.setSubject(ASUNTO_MIGRADO);
            htmlMail.setHtmlMsg(content);
            htmlMail.setCharset("UTF-8");
            htmlMail.setStartTLSEnabled(true);
            htmlMail.setSSLOnConnect(false);
            htmlMail.send();
