...
public class CmisBaseServiceImpl implements CmisBaseService {
    ...
    public CmisBaseServiceImpl() throws KeyManagementException, NoSuchAlgorithmException {
    }
...
//        return clientHttpRequestFactory;
    ...
    public CmisFolder createFolder(String nameFolder) throws Exception {
        ...
        cmisFolder.setPath(newFolder.getPath());
        cmisFolder.setMensaje("Se ha creado un folder en " + newFolder.getPath());
        return cmisFolder;

    }

    public CmisFolder SubCreateFolder(String parentFolderId,String nameFolder) throws Exception {
        Session session = this.getSessionRepositoryToken();

        Folder rootFolder = (Folder) session.getObject(parentFolderId);
        Folder subFolder = null;
        CmisFolder cmisFolder = new CmisFolder();
        System.out.println(rootFolder.getPath());
        try {
            // Making an assumption here that you probably wouldn't normally do
            subFolder = (Folder) session.getObjectByPath(rootFolder.getPath() + "/" + nameFolder);
            System.out.println("Folder already existed!");
            cmisFolder.setId(subFolder.getId());
            cmisFolder.setMensaje("La carpeta " +rootFolder.getPath() + "/" + nameFolder+ " existe!");
            return cmisFolder;

        } catch (CmisObjectNotFoundException onfe) {
            Map<String, Object> props = new HashMap<String, Object>();
            props.put("cmis:objectTypeId",  "cmis:folder");
            props.put("cmis:name", nameFolder);
            ...
            cmisFolder.setId(subFolder.getId());
            cmisFolder.setMensaje("Se ha creado un folder en " + subFolder.getPath());
        }
        return cmisFolder;
    }
    ...
    private CmisFolderSession obtenerFolderSession(String nameFolder) throws Exception {
        Session session = this.getSessionRepositoryToken();
        Folder root = null;
        String flag = nameFolder.substring(0,1);
        CmisFolderSession cmisFolderSession = new CmisFolderSession();
        cmisFolderSession.setSession(session);
        if(flag.equals("/")){
                // Making an assumption here that you probably wouldn't normally do
                root = (Folder) session.getObject(nameFolder.substring(1));
                CmisFolder cmisFolder = new CmisFolder();
                cmisFolderSession = new CmisFolderSession();
                cmisFolderSession.setSession(session);
                System.out.println("Folder already existed!");
                cmisFolder.setId(root.getId());
                cmisFolder.setMensaje("Se ha creado un folder en " + root.getPath());
    ...
    public CmisFile createDocumento(String nameFolder, MultipartFile file,String folderName,String fileName) throws Exception {
        ...
        String contentTypes = "";
        if(fileName!= null){
            contentTypes = BASE64DecodedMultipartFile.mimesType(fileName);
            extension = "."+fileName;
        }else{
        ...
        //EAAR fecha
        String fechaAutogenerada = UUID.randomUUID().toString();
        fileNewName = fechaAutogenerada;
        String fileNameDb =  fileNewName;
        String extensionDb = extension.replace(".", "");
        //
        log.error("createDocumento 02 fileNewName: " + fileNewName);
        ...
        long mySize = file.getBytes().length; //file.getSize();
        String myType =contentTypes == ""  ? file.getContentType() : contentTypes;

        ContentStream contentStream = session.getObjectFactory()
                .createContentStream(fileNewName, mySize, myType, stream);
        Document doc = null;
        try {
            doc = folder.createDocument(properties, contentStream, VersioningState.MAJOR);
        } catch (Exception ex) {
    ...
    public CmisFile createDocumento(String nameFolder, MultipartFile file, String fileName) throws Exception {
        ...
        log.error("createDocumento 00 extension: " + extension);
        String fileNewName = fileName.replace(extension, "");
        log.error("createDocumento 01 fileNewName: " + fileNewName);
        ...
        //fileNewName = fileNewName + "_" + fechaAutogenerada;
        fileNewName = fileNewName + "_" + uuid.toString();
        ...
        long mySize = file.getBytes().length; //file.getSize();
        String myType = "txt";//file.getContentType();
    ...
    public FileMultipart getDocumentMultipartByIdAdjunto(String archivoId, String extensionFile, String nameFile) throws Exception {
        ...
        if (contentStream != null) {
            ...
            fileMultipart.setNombreDocumento(nameFile+"."+extensionFile);
            fileMultipart.setExtension(extensionFile);
    ...
    public ByteArrayOutputStream getDocumento(String archivoId) throws Exception {
        ...
        Session session = this.getSessionRepositoryToken();
        CmisObject cmisObject = session.getObject(archivoId);
        byte[] bytes = null;
        if(cmisObject instanceof Document){
            Document document = (Document) session.getObject(archivoId);
            ContentStream contentStream = document.getContentStream();
            InputStream inputStream = contentStream.getStream();
            try {
                bytes = IOUtils.toByteArray(inputStream);
            } catch (IOException ex) {
                throw new PortalException("No se pudo convertir el archivo a base64");
            }
        }else {
            LogTransaccion logTransaccion = new LogTransaccion();
            logTransaccion.setRespuestaTexto("Error");
            logTransaccion.setRespuestaCodigo("");
            logTransaccion.setTipoRegistro("Descarga CMIS");
...
//            return  responseCrearDocumentoDto;
    ...
    public Object crearDocumentoCuentaBancaria(Integer id, AdjuntoCargaDto adjuntoCargaDto, Proveedor p) throws Exception {
        ...
        //Enviar data con base64
        String accessToken = getAutenticacionRs();
        Object dataResponse = getCrearDocumentoCuentaBancaria(id, accessToken, proveedor, adjuntoCargaDto);
        System.out.println("");
        return dataResponse;
    ...
    public String getAutenticacionRs() throws Exception {
        ...
        try {
            ...
            logTransaccion.setEnvioTrama(requestBody.toString());
    ...
    public Object getCrearDocumento(String accessToken, Proveedor p, AdjuntoCargaDto dataRequest) throws Exception {
        ...
        xmlString += "<Creador><![CDATA[" + p.getRazonSocial() + "]]></Creador>";
        ...
        xmlString += "<Razon_Social><![CDATA[" + p.getRazonSocial() + "]]></Razon_Social>";
    ...
    public Object getCrearDocumentoCuentaBancaria(Integer id, String accessToken, Proveedor p, AdjuntoCargaDto dataRequest) throws Exception {
        LogTransaccion logTransaccion = new LogTransaccion();
        //obtener tipo de documento
        MtrTipoDocumento mtrTipoDocumento = this.mtrTipoDocumentoRepository.findByCodigoTipoDocumento(dataRequest.getCodigoDocumento());
        ...
        if (!responseCrearDocumentoDto.getIDDocumento().equals("") || !responseCrearDocumentoDto.getIDDocumento().equals(null)) {
            ...
            ProveedorCuentaBancaria proveedorCuentaBancaria = this.proveedorCuentaBancariaRepository.getById(id);
            proveedorCuentaBancaria.setArchivoId(responseCrearDocumentoDto.getIDDocumento().toString());
            proveedorCuentaBancaria.setArchivoNombre(dataRequest.getNombreDocumento());
            proveedorCuentaBancaria.setRutaAdjunto("archivo_" + responseCrearDocumentoDto.getIDDocumento().toString());
            proveedorCuentaBancaria.setArchivoTipo(dataRequest.getFileExtencion());
            proveedorCuentaBancaria.setProveedor(p);
            this.proveedorCuentaBancariaRepository.save(proveedorCuentaBancaria);
    ...
    public Object crearDocumentoLicitacion(LicitacionAdjuntoBase64Dto item, Licitacion licitacionGen) throws Exception {
        ...
        xmlString += "<Contacto_Comercial></Contacto_Comercial>";
        xmlString += "<Correo_Electronico></Correo_Electronico>";
        xmlString += "<Creador><![CDATA[" + licitacionGen.getUsuarioCreacion() + "]]></Creador>";
        xmlString += "<Direccion></Direccion>";
        ...
        xmlString += "<Nombre_Concurso>" + licitacionGen.getNombreLicitacion() + "</Nombre_Concurso>";
        xmlString += "<Nombre_Documento>" + mtrTipoDocumento.getDescripcion() + "</Nombre_Documento>";
        xmlString += "<Pais></Pais>";
        xmlString += "<Pedido></Pedido>";
        xmlString += "<Provincia></Provincia>";
...
//        responseCrearDocumentoDto =  g.fromJson(respuestaBody.toString(), ResponseCrearDocumentoDto.class);
    ...
    public Object crearDocumentoActaSustento(AdjuntoActaSustentoBase64Dto sustentoBase64Dto, ActaSustento actaSustento) throws Exception {
        ...
        //xml
        String xmlString = "<Formulario xmlns:i='http://www.w3.org/2001/XMLSchema-instance' xmlns='http://schemas.datacontract.org/2004/07/webAuthentic.Models'>";
        xmlString += "<Acta_Sustentos>" + actaSustento.getAnioActaSustento() + "-" + actaSustento.getNumeroActaSustento() + "</Acta_Sustentos>";
        xmlString += "<Codigo_Aceptacion>" + actaSustento.getAnioActaSustento() + "-" + actaSustento.getNumeroActaSustento() + "</Codigo_Aceptacion>";
        xmlString += "<Codigo_Concurso>" + actaSustento.getAnioActaSustento() + "-" + actaSustento.getNumeroActaSustento() + "</Codigo_Concurso>";
        xmlString += "<Contacto_Comercial></Contacto_Comercial>";
        xmlString += "<Correo_Electronico></Correo_Electronico>";
        xmlString += "<Creador></Creador>";
        xmlString += "<Direccion></Direccion>";
        ...
        xmlString += "<Nombre_Concurso></Nombre_Concurso>";
        xmlString += "<Nombre_Documento>" + sustentoBase64Dto.getMtrTipoDocumento() + "</Nombre_Documento>";
        xmlString += "<Pais></Pais>";
...
//        responseCrearDocumentoDto =  g.fromJson(respuestaBody.toString(), ResponseCrearDocumentoDto.class);
    ...
    public Object crearDocumentoFichaActaSustento(AdjuntoActaSustentoBase64Dto sustentoBase64Dto, ActaSustento actaSustento) throws Exception {
        ...
        String codigoSap = actaSustento.getCodigoSapEm() != null ? actaSustento.getCodigoSapEm() : actaSustento.getCodigoSapHes() != null ? actaSustento.getCodigoSapHes() : actaSustento.getNumeroActaSustento();
        xmlString += "<Codigo_Aceptacion>" + codigoSap + "</Codigo_Aceptacion>";
