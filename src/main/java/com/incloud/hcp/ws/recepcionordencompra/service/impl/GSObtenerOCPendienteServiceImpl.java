package com.incloud.hcp.ws.recepcionordencompra.service.impl;
...
import com.incloud.hcp.ws.recepcionordencompra.service.builder.ObtenerOCPendienteGSBuilder;
...
@Service
public class GSObtenerOCPendienteServiceImpl implements GSObtenerOCPendienteService {
    ...
    @Override
    public ObtenerOCPendienteResponse obtenerOCPendiente(int idEmpresa, String fechaInicio, String fechaFinal, String iD_Agenda) {
        ...
        OkHttpClientSync mOkHttpClientSync = new OkHttpClientSync("OCPENDIENTE_OBTENER");
        ...
        try {
            ...
            ObtenerOCPendienteResponse responseWS = new ObtenerOCPendienteResponse();
            // 2. Obtenemos los datos de entrada del ws en formato json y se crea un RequestBody
            String jsonInput = ObtenerOCPendienteGSBuilder.newBuilder(idEmpresa,fechaInicio,fechaFinal, iD_Agenda).build();
            ...
            Request request = mOkHttpClientSync.getRequestPostAuthorization(String.format("%s/%s",url, GSConstant.PATH_API_GS_OCPENDIENTE_OBTENER), body, key);
            logger.info("Enviando trama para obtener OC Pendientes -> {}", jsonInput);
            ...
            if(response.isSuccessful() && !response.message().contains("ya existe")){
                ...
                if(!String.valueOf(result.get("status")).equals("200")) {
                    result.put("body", new ObtenerOCPendienteBodyResponse());
                    Gson gson = new GsonBuilder().create();
                    respuestaJson = gson.toJson(result);
                }
                responseWS = g.fromJson(respuestaJson, ObtenerOCPendienteResponse.class);
            } else {
            ...
            responseWS.setMessageException("Error al obtener las OC pendientes: " + ex.getMessage());
