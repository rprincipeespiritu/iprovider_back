package com.incloud.hcp.ws.obtenersolicitudcompra.service.impl;
...
import com.incloud.hcp.ws.obtenersolicitudcompra.bean.ObtenerCompraBodyResponse;
...
import com.incloud.hcp.ws.obtenersolicitudcompra.service.builder.ObtenerCompraGSBuilder;
...
@Service
public class GSObtenerSolicitudCompraServiceImpl implements GSObtenerSolicitudCompraService  {
    ...
    public ObtenerCompraResponse obtenerSolicitudCompra(int idEmpresa, String fechaInicio, String fechaFinal) {
        ...
        OkHttpClientSync mOkHttpClientSync = new OkHttpClientSync("SOLICITUD_COMPRA");
        ...
        try {
            ...
            ObtenerCompraResponse responseWS = new ObtenerCompraResponse();
            // 2. Obtenemos los datos de entrada del ws en formato json y se crea un RequestBody
            String jsonInput = ObtenerCompraGSBuilder.newBuilder(idEmpresa,fechaInicio,fechaFinal).build();
            ...
            Request request = mOkHttpClientSync.getRequestPostAuthorization(String.format("%s/%s",url, GSConstant.PATH_API_GS_COMPRA_SOLICITUD), body, key);
            ...
            if(response.isSuccessful() && !response.message().contains("ya existe")){
                ...
                if(!String.valueOf(result.get("status")).equals("200")) {
                    result.put("body", new ObtenerCompraBodyResponse());
            ...
            responseWS.setMessageException("Error al obtener la compra: " + ex.getMessage());
